...
class UserService:
    ...
    async def list_users(self, limit: int = 50, offset: int = 0, search: Optional[str] = None, is_active: Optional[bool] = None):
        users = await self.user_repo.list(limit=limit, offset=offset, search=search, is_active=is_active)
        total = await self.user_repo.count(search=search, is_active=is_active)
        return users, total
