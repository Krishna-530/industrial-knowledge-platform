...
class JobService:
    ...
    async def list_jobs(self, limit: int = 50, offset: int = 0, status: Optional[str] = None, job_type: Optional[str] = None, search: Optional[str] = None) -> tuple[List[Job], int]:
        total = await self.repo.count_jobs(status, job_type, search)
        items = await self.repo.list_jobs(limit, offset, status, job_type, search)
        return items, total
    ...
    async def get_job_details(self, job_id: UUID) -> Dict[str, Any]:
        ...
        details = {"job": job, "document": None, "document_content": None, "document_version": None}
        ...
        if version_id:
            ...
            stmt = select(DocumentContent).where(DocumentContent.document_version_id == version_id)
            res = await self.session.execute(stmt)
            content = res.scalars().first()
            if content:
                details["document_content"] = content
                
            stmt2 = select(DocumentVersion).where(DocumentVersion.id == version_id)
            res2 = await self.session.execute(stmt2)
            version = res2.scalars().first()
            if version:
                details["document_version"] = version
                
        return details

    async def retry_job(self, job_id: UUID, admin_id: UUID) -> Job:
        job = await self.repo.get_by_id(job_id)
        if not job:
            from core.exceptions.base import NotFoundError
            raise NotFoundError("Job not found")
            
        if job.status != JobStatus.FAILED and not job.is_dead_letter:
            from core.exceptions.base import AppException
            raise AppException(status_code=400, message="Only FAILED or DEAD LETTER jobs can be retried.")
