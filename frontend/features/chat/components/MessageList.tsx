...
export function MessageList({ messages, scrollContainerRef }: MessageListProps) {
  ...
  if (messages.length === 0) {
    return (
      <div 
        ...
        className="flex-1 overflow-y-auto min-h-0 p-4 sm:p-6 flex flex-col items-center justify-center text-center space-y-4"
      >
  ...
  return (
    <>
      <div 
        ref={scrollContainerRef}
        className="flex-1 overflow-y-auto min-h-0 p-4 sm:p-6 space-y-2"
      >
        {messages.map((msg) => (
          <MessageBubble 
            key={msg.id} 
